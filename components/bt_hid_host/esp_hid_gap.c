...
#define HID_KNOWN_BDA_MAX 2
static uint8_t s_ble_known_bda[HID_KNOWN_BDA_MAX][6];
static size_t s_ble_known_bda_num = 0;
...
{
    for (size_t i = 0; i < s_ble_known_bda_num; i++) {
        if (memcmp(bda, s_ble_known_bda[i], sizeof(s_ble_known_bda[i])) == 0) {
            return true;
        }
    }
    return false;
...
static void handle_ble_device_result(struct ble_scan_result_evt_param *scan_rst)
{
    ...
    char name[64] = {0};
    bool is_known = ble_scan_is_known_bda(scan_rst->bda);
    ...
    if (uuid == ESP_GATT_UUID_HID_SVC || is_known) {
...
static void ble_gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_AUTH_CMPL_EVT:
        ...
        if (s_passkey_cb) {
            s_passkey_cb(false, 0, s_passkey_cb_ctx);
        }
        break;
    ...
    case ESP_GAP_BLE_PASSKEY_NOTIF_EVT: // ESP_IO_CAP_OUT
        ...
        if (s_passkey_cb) {
            s_passkey_cb(true, param->ble_security.key_notif.passkey, s_passkey_cb_ctx);
        }
        break;
...
static void handle_ble_device_result(const struct ble_gap_disc_desc *disc)
{
    ...
    uint8_t adv_name_len = 0;
    bool is_known = ble_scan_is_known_bda(disc->addr.val);
...
esp_err_t esp_hid_scan_set_known_bda(const uint8_t (*bda_list)[6], size_t num)
{
    if (num > HID_KNOWN_BDA_MAX) {
        num = HID_KNOWN_BDA_MAX;
    }
    s_ble_known_bda_num = 0;
    for (size_t i = 0; i < num; i++) {
        memcpy(s_ble_known_bda[s_ble_known_bda_num], bda_list[i], sizeof(s_ble_known_bda[0]));
        s_ble_known_bda_num++;
    }
    return ESP_OK;
}
