...
int lcd1602_set_custom_char(lcd1602_context context, uint8_t slot, const uint8_t pattern[8])
{
   lcd1602_t *c = (lcd1602_t *) context;

   if(slot > 7 || pattern == NULL)
      return -1;

   if(lcd1602_write_byte(c, LCD1602_CMD_SET_CGRAM_ADDR | ((slot & 0x07) << 3), false, 0) != 0)
      return -1;

   for(uint8_t i = 0; i < 8; ++i)
   {
      if(lcd1602_write_byte(c, pattern[i] & 0x1f, true, 0) != 0)
         return -1;
   }
